...
namespace CodeStage.AntiCheat.Examples
{
	...
	public class ObscuredPerformanceTests : MonoBehaviour
	{
		...
		{
			logBuilder.Length = 0;
			logBuilder.AppendLine(ACTkConstants.LOG_PREFIX + "<b>[ Performance tests ]</b>");
		...
		private void TestDouble()
		{
			...
			if (Mathf.Abs((float)dummy) > 0.00001d) {}
			if (Mathf.Abs((float)obscured) > 0.00001d) {}
			if (Mathf.Abs((float)notObscured) > 0.00001d) {}
		}
		...
		private void TestFloat()
		{
			...
			if (Mathf.Abs(dummy) > 0.00001f) {}
			if (Mathf.Abs(obscured) > 0.00001f) {}
			if (Mathf.Abs(notObscured) > 0.00001f) {}
		}
