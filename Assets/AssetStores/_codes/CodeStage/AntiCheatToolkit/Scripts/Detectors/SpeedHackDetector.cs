...
namespace CodeStage.AntiCheat.Detectors
{
	...
	[HelpURL(ACTkConstants.DOCS_ROOT_URL + "class_code_stage_1_1_anti_cheat_1_1_detectors_1_1_speed_hack_detector.html")]
	public class SpeedHackDetector : ActDetectorBase
	{
		...
		private void Update()
		{
			...
			var timeCheated = Mathf.Abs((ticksFromStart) / (float)TICKS_PER_SECOND - (vulnerableTime - vulnerableTimeOnStart)) > THRESHOLD_FLOAT;
			...
			if (ticksCheated || timeCheated)
			{
				...
				{
#if ACTK_DEBUG_ENABLED
					Debug.LogWarning(ACTkConstants.LOG_PREFIX + "SpeedHackDetector: detection! Allowed false positives left: " + (maxFalsePositives - currentFalsePositives), this);
#endif
			...
			{
#if ACTK_DEBUG_ENABLED
				Debug.LogWarning(ACTkConstants.LOG_PREFIX + "SpeedHackDetector: success shot! Shots till cool down: " + (coolDown - currentCooldownShots), this);
#endif
				currentCooldownShots++;
				if (currentCooldownShots >= coolDown)
				{
#if ACTK_DEBUG_ENABLED
					Debug.LogWarning(ACTkConstants.LOG_PREFIX + "SpeedHackDetector: cool down!", this);
#endif
